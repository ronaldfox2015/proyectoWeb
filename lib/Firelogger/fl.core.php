...
//    if (!defined('FIRELOGGER_PASSWORD')) define('FIRELOGGER_PASSWORD', 'As_qsMO:(bv2xbWEU~a=Ax>M2uhVx[a]');
    ...
    class FireLogger
    {
        ...
        {
            $config = include APPLICATION_PATH . '/config/autoload/firelogger.global.php';
            $enabled = isset($config['firelogger']['enabled']) ? $config['firelogger']['enabled'] : FALSE;
            return $enabled;            
        }
        ...
        {
            if (!FireLogger::getConfig()) return; // no-op
        ...
        {
            // FireLogger::$codeErr = (int) $errno;
            // echo '<br>firelogger_error_handler -> code:'.FireLogger::$codeErr."<br>$errstr";
        ...
        {
            // echo '<br>handler -> errcode:'.FireLogger::$codeErr;
            // if(FireLogger::$codeErr==8) return;
    ...
    if (!defined('FIRELOGGER_NO_PASSWORD_CHECK') && defined('FIRELOGGER_PASSWORD') && FireLogger::getConfig()) {
    ...
    if (!defined('FIRELOGGER_NO_OUTPUT_HANDLER')) {
        if (FireLogger::getConfig()) ob_start(); // start output buffering (in case firelogger should be enabled)
    }
...
//    }
    ...
    if (FireLogger::getConfig()) register_shutdown_function(array('FireLogger', 'handler'));
